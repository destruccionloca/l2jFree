...
public class FrintezzaManager
{
    ...
    private static L2MonsterInstance	demon1,
    									demon2,
    									demon3,
    									demon4;    								
...
/************************************** Initial Functions **************************************/
    ...
    {
        if (_instance == null) _instance = new FrintezzaManager();
...
/******************************* Spawn Manager Creates all spawns *********************************/    
     ...
     * @param templateId int value of the monster template id number
     * @param x int value of the X position
     * @param y int value of the Y position
     * @param z int value of the Z position
     * @param heading int value of where is the monster facing to...
     * @param respawnDelay int value of the respawn of this L2Spawn
     ...
     * @param pc L2PcInstance of the player
    ...
    /**
     * Checks whether the players were annihilated.
     * If all players in the lair are dead, return boolean 
     * <b>true</b> else returns boolean <b>false</b> 
     */ 
    ...
    public void banishesPlayers()
    {
    	...
    	for(L2PcInstance pc : _PlayersInLair)
    	{
    		...
    		if(checkIfInZone(pc))
    		{
        		...
        		pc.teleToLocation( 
        				_banishmentLocation[loc].getX() + driftX,
        				_banishmentLocation[loc].getY() + driftY,
        				_banishmentLocation[loc].getZ());
    		}
    	}
	...
	private class OnPlayersAnnihilatedTask implements Runnable
	{
		...
		public OnPlayersAnnihilatedTask()	{ /* Nothing */ }
...
/************************** Starting the battle with Frintezza + co. ******************************/ 
     ...
     * @param task Future task that is still active
    ...
    /**
     * I noticed that if the players do not stand at a certain position,
     * they can not watch the entire movie, so I set them to the center during
     * the entire movie.  
     */
...
/**************************** Initialize a movie and spawn the monsters ************************/
    ...
    /**
     * Spawns Frintezza, the weak version of Scarlet Van Halisha, the minions, and all that
     * is shown in a movie to the observing players.
     */ 
     ...
     * 1. Song, cast on Frintezza, for the music to play (skill 5007, levels 1-5)
     *    Each level is a different tune =)
     * 2. Visual Effect, cast on targets, to show the effect (skill 5008, levels 1-5)
     *    Each level has a different animation effect and different targets and purpose
     * 3. Actual skill, which is different since NCSoft has a different skill system. 
     *    I used other skill Ids to implement these effects:
     *    song effects:
    ...
    private class SongEffectLaunched implements Runnable
    {
    	...
    	/**
    	 * Constructor
    	 * @param targets 	- song's targets L2Object[]
    	 * @param song		- song id 1-5
    	 * @param previousHp	- Frintezza's HP when he started to play
    	...
    	public void run()
    	{
    		...
    		SongEffectLaunched songLaunched = new SongEffectLaunched( _targets , _song , (int)frintezza.getStatus().getCurrentHp() , _currentTime + _intervalOfFrintezzaSongs/10);
    	 ...
    	 * @param target - L2Character affected by the music
    	 */
     ...
     * @param target L2Character who's HP Regeneration is decreased.
     */
     ...
     * @param skill L2Skill to calculate it's duration
     * @param target L2Character to calculate the duration on it
    ...
    /**
     * This function simulates the functions of "Angelic Icon". Takes the 5th function which is 
     * gainHp*0.2 and adds it to the skill id 5007, level 4 to decrease gainHP 
    ...
    /**
     * Further implementation into the core is needed. But this will do for now ;]
     * Class needed to implement the start Frintezza dance+stun effect on a target.
     * @author Darki699
     */
...
/************************ Minion Control Attack + Respawn + Polymorph ****************************/
    ...
    /**
     * Initializes the Attack Listeners for <b>all</b> monsters in this zone.
     * Sends a thread loop (tasks canceled ofcourse) with the mob to be listened to, and
    ...
    /**
     * Class is recalled at an interval for a monster's life time.
     * Once the monster is <b>deleted</b> (null), this class is not called anymore
     * If the monster is <b>dead</b>, this class is still called at the interval,
     * but it does nothing until next respawn.
     * If the monster is <b>alive</b> and is being attacked, it "tells" the other
     * monsters that it's attacked. 
     * @author Darki699
     */
	...
	private class attackerListener implements Runnable
    {
    	...
    	public void run()
    	{
    		...
    		/*if (target instanceof L2PcInstance && (((L2PcInstance)target).isInvul() || ((L2PcInstance)target).getAppearance().getInvisible()))
    		{
    			...
    			_mob.abortCast();
    			_mob.setTarget(null);
    			_mob.getKnownList().getKnownPlayers().remove((L2PcInstance)target);
    			return;
    		}*/
     ...
     * If the dead boss is Scarlet or Frintezza, we do a bossesAreDead() check to see if 
     * both Frintezza and Scarlet are dead.
     * @param mob - L2BossInstance that is (or is set as) dead.
     */
    ...
    /**
     * controls the assistance for all 3 bosses:
     * 1. if Frintezza needs help, all (including Scarlet van Halisha) help him
     * 2. if Scarlet needs help, all (including Frintezza) come to his help
     * 3. if Strong Scarlet is already spawned, then he teleports to help Frintezza
     * @param L2Character attacker  - The player that attacked the boss
     * @param int hate 				- Damage hate to add to the attacker
     * 1. Frintezza 		adds 200 hate
     * 2. Weak Scarlet 		adds 100 hate
     * 3. Stronger Scarlet 	adds 125 hate
     * 4. Strongest Scarlet adds 150 hate
     * 5. Portraits 		adds  50 hate
     * 6. Demons			adds   1 hate
     */
	...
	/**
     * Checks on a killed demon to set it's respawn time 
     * (only done if the <b>Portrait</b> of <b>Demon</b> was not killed)
     * @param mob - L2MonsterInstance mob of the Demon that was killed
     * @return int value of the demon that should be respawned. -1 is returned 
     * if the demon should not respawn now. 
     */
    ...
    /**
     * Listens to the weaker Scarlet Van Halisha HP status. If it's weakened enough, we spawn
     * the Stronger version of Scarlet and delete the weaker one.
     */
	...
	public void weakScarletHpListener()
    {
    	...
    	double 	curHp = weakScarlet.getStatus().getCurrentHp(),
    			maxHp = weakScarlet.getMaxHp();
	...
	/**
     * Does the 3rd and last polymorph for Scarlet Van Halisha. 
     * Now he looks entirely different... (he is different) 
     */
    ...
    /**
     * Starts the skill effects for Scarlet Van Halisha. He moves like the wind ;]
     * Continuous skills cast at _intervalOfRetarget
     * @author Darki699
     */
	...
	private class doSkill implements Runnable
    {
    	 ...
    	 * @param caster	- the monster
    	 * @param skill		- the skill to animate
    	 * @param interval	- the time between leaps
    	 * @param range		- the range minimum to teleport
    	 */
		...
		public void run()
    	{
			...
			try
			{
	    		...
	    		if (	!_caster.isInsideRadius(x, y, _range, false) 
	    				&& 
	    				tempTarget instanceof L2PcInstance 
	    				&& 
	...
	private class ReTarget implements Runnable
    {
    	...
    	public void run()
    	{
    		...
    		if (_mob == null || _mob.isDead() || 
    			_PlayersInLair == null || _PlayersInLair.isEmpty())
    		{		
...
/************************************* End of re-target class ****************************************/    
    ...
    /**
     * Class ends the activity of the Bosses after a interval of time
     * Exits the battle field in any way ... 
     * @author Darki699
     */
    private class ActivityTimeEnd implements Runnable
    {
    	public ActivityTimeEnd()	{ /* Nothing */ }
    ...
    /**
     * Creates a thread to initialize Frintezza again... until this loops ends, no one can 
     * enter the lair.
     */
    ...
    /**
     * Updates knownlist for the monster.
     * Updates players in the room list.
     * Updates players in the room known list.
     * @param L2NpcInstance monster
     */ 
