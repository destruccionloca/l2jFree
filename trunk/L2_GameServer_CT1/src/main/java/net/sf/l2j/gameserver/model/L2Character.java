...
public abstract class L2Character extends L2Object
{
	...
	private boolean doAttackHitByPole(Attack attack, int sAtk)
	{
		...
		for (L2Object obj : getKnownList().getKnownObjects().values())
		{
			...
			if (obj instanceof L2Character)
			{
				...
				if (Math.abs(angleChar - angleTarget) > maxAngleDiff && Math.abs((angleChar + 360) - angleTarget) > maxAngleDiff && // Example: char is at 1
																																	// degree and target is at
																																	// 359 degree
	...
	public void doCast(L2Skill skill)
	{
		...
		if (FortressSiege._started
				...
				&& (skill.getTargetType() == L2Skill.SkillTargetType.TARGET_CLAN || skill.getTargetType() == L2Skill.SkillTargetType.TARGET_ALLY || skill
						.getTargetType() == L2Skill.SkillTargetType.TARGET_ENEMY_ALLY))
		...
		if (weaponInst != null && skill.isMagic() && !forceBuff && skill.getTargetType() != SkillTargetType.TARGET_SELF)
		{
					...
					|| (weaponInst.getChargedSpiritshot() == L2ItemInstance.CHARGED_SPIRITSHOT))
			{
				...
				if ((skill.getSkillType() == SkillType.BUFF) || (skill.getSkillType() == SkillType.MANAHEAL) || (skill.getSkillType() == SkillType.RESURRECT)
	 ...
	 * 
	 * @param targets -
	 *            Target list L2Object[] received
	 * @param offensive -
	 *            true to check Offensive skills or hits from players IN THE EVENT as well. false to check ALL interference
		...
		try
		{
			...
			for (int x = 0; x < targets.length; x++)
			{
				...
				if ((TvT._started && !Config.TVT_ALLOW_INTERFERENCE) || (CTF._started && !Config.CTF_ALLOW_INTERFERENCE)
						|| (DM._started && !Config.DM_ALLOW_INTERFERENCE) || (FortressSiege._started && !Config.FortressSiege_ALLOW_INTERFERENCE))
				{
	...
	{
		return isFakeDeath() || !(getStatus().getCurrentHp() > 0.5);
	}
	...
	public EffectCharge getChargeEffect()
	{
		...
		for (L2Effect e : effects)
		{
			if (e.getSkill().getSkillType() == L2Skill.SkillType.CHARGE) { return (EffectCharge) e; }
		}
		return null;
	}
	...
	public void setTarget(L2Object object)
	{
		...
		if (object == null)
		{
			...
			/*
			 * if (isAttackingNow() && getAI().getAttackTarget() == _target) { abortAttack();
			 * 
			 * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
			 * 
			 * if (this instanceof L2PcInstance) { sendPacket(new ActionFailed()); SystemMessage sm = new SystemMessage(SystemMessageId.S1_S2);
			 * sm.addString("Attack is aborted"); sendPacket(sm); } }
			 * 
			 * if (isCastingNow() && canAbortCast() && getAI().getCastTarget() == _target) { abortCast();
			 * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE); if (this instanceof L2PcInstance) { sendPacket(new ActionFailed()); SystemMessage sm =
			 * new SystemMessage(SystemMessageId.S1_S2); sm.addString("Casting is aborted"); sendPacket(sm); } }
			 */
		}
		_target = object;
	}
	...
	protected void moveToLocation(int x, int y, int z, int offset)
	{
		...
		if (Config.GEODATA && !this.isFlying()) // currently flying characters not checked
		{
			...
			if (Config.GEO_PATH_FINDING && originalDistance - distance > 100 && distance < 2000)
			{
				...
				if (this instanceof L2PlayableInstance || this.isInCombat())
				{
					...
					else
					{
						...
						/*
						 * Location destiny = GeoData.getInstance().moveCheck(curX, curY, curZ, x, y, z); if (destiny.getX() != x || destiny.getY() != y) {
						 * m.geoPath = null; getAI().stopFollow(); getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE); return; }
						 */
			...
			// If no distance to go through, the movement is canceled
			if (distance < 1
					&& (Config.GEO_PATH_FINDING || this instanceof L2PcInstance || (this instanceof L2Summon && !((L2Summon) this).getFollowStatus()) || this instanceof L2RiftInvaderInstance))
			{
				sin = 0;
				cos = 1;
				distance = 0;
	...
	{
		if (!isInFunEvent() || !attacker.isInFunEvent()) { return isInsidePeaceZone(attacker, this); }
		return false;
	}
	...
	public void callSkill(L2Skill skill, L2Object[] targets)
	{
		try
		{
			...
			if (skill.getTargetType() == SkillTargetType.TARGET_HOLY && this instanceof L2PcInstance
					&& FortressSiege.checkIfOkToCastSealOfRule((L2PcInstance) this))
			{
				FortressSiege.Announcements(getName() + " finished casting Seal Of Ruler. " + ((L2PcInstance) this)._teamNameFOS + " has taken "
						+ FortressSiege._eventName + "!");
