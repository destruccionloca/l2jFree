 ...
 * @author  Imper1um
 */
public class LeaderboardTotalMonKills implements Leaderboard {
   private static String leaderboardDescription = "Total Monster Kills";
   private int id = -1;
   private FastList<LeaderboardEntry> _leaderboard;
   
   public LeaderboardTotalMonKills() {
   ...
   public void update(Connection c, ResultSet rs) throws SQLException {
       ...
       while (rs.next()) {
           if (_leaderboard.getLast() == null) { //No Entries, yet.
               ...
               _leaderboard.add(new LeaderboardEntry(s, rs.getDouble("totalMonKills")));
           } else {
               int i = -1;
               double sc = rs.getDouble("totalMonKills");
               boolean pass = true;
