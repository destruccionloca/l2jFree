 ...
 * @author  Imper1um
 */
public class LeaderboardTotalPlayerDeaths implements Leaderboard {
   private static String leaderboardDescription = "Total Player Deaths";
   private int id = -1;
   private FastList<LeaderboardEntry> _leaderboard;
   
   public LeaderboardTotalPlayerDeaths() {
   ...
   public void update(Connection c, ResultSet rs) throws SQLException {
       ...
       while (rs.next()) {
           if (_leaderboard.getLast() == null) { //No Entries, yet.
               ...
               _leaderboard.add(new LeaderboardEntry(s, rs.getInt("totalPlayerDeaths")));
           } else {
               int i = -1;
               int sc = rs.getInt("totalPlayerDeaths");
               boolean pass = true;
