    ...
    public int addSpawn(int npcId, int x, int y, int z, boolean randomOffset)
    {
        try {
            ...
            if (template1 != null)
            {
                ...
                if ((x == 0) && (y == 0))
                {
                	_log.warn(getPlayer().getName() + " requested quest spawn with loc 0,0.  Loc is being adjusted");
                    ...
                    if ((x == 0) && (y == 0))
                    {
                    	_log.fatal("Failed to adjust bad locks for quest spawn requested by "+getPlayer().getName() + "!  Spawn aborted!");
                    	return 0;
                    }
                }
                spawn.setLocx(x);
