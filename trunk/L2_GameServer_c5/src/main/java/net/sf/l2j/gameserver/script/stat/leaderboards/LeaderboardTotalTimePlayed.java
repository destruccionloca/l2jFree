 ...
 * @author  Imper1um
 */
public class LeaderboardTotalTimePlayed implements Leaderboard {
   private static String leaderboardDescription = "Total Time Played";
   private int id = -1;
   private FastList<LeaderboardEntry> _leaderboard;
   
   public LeaderboardTotalTimePlayed() {
   ...
   public void update(Connection c, ResultSet rs) throws SQLException {
       ...
       while (rs.next()) {
           if (_leaderboard.getLast() == null) { //No Entries, yet.
               ...
               _leaderboard.add(new LeaderboardEntry(s, rs.getDouble("totalPlayTime")));
           } else {
               int i = -1;
               double sc = rs.getDouble("totalPlayTime");
               boolean pass = true;
