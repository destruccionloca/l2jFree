 ...
 * @author  Imper1um
 */
public class LeaderboardTotalKarma implements Leaderboard {
   private static String leaderboardDescription = "Total Karma";
   private int id = -1;
   private FastList<LeaderboardEntry> _leaderboard;
   
   public LeaderboardTotalKarma() {
   ...
   public void update(Connection c, ResultSet rs) throws SQLException {
       ...
       while (rs.next()) {
           if (_leaderboard.getLast() == null) { //No Entries, yet.
               ...
               _leaderboard.add(new LeaderboardEntry(s, rs.getDouble("totalKarma")));
           } else {
               int i = -1;
               double sc = rs.getDouble("totalKarma");
               boolean pass = true;
