...
/*        public void doCast(L2Skill skill) 
    ...
    public void setTemplate(ClassId newclass, boolean female)
    {
        super.setTemplate(CharTemplateTable.getInstance().getTemplate(newclass, female));
    }
    ...
    public QuestState[] getQuestsForAttacks(L2NpcInstance npc)
    {
        ...
        for (QuestState qs : _quests.values())
        {
            ...
            if (qs.waitsForAttack(npc))
            {
                ...
                if (states == null) states = new QuestState[] {qs};
                else states = addToQuestStateArray(states, qs);
            }
        }
    ...
    private void showQuestWindow(String questId, String stateId)
    {
        ...
        {
            if (Config.DEBUG)
                _log.fine("Showing quest window for quest " + questId + " state " + stateId
                    + " html path: " + path);
    ...
    public void setInPvpZone(boolean inPvpZone, boolean warnPlayer)
    {
        if (_inPvpZone != inPvpZone)
        {
            ...
            if (warnPlayer) if (_inPvpZone) sendPacket(new SystemMessage(283));
            else sendPacket(new SystemMessage(284));
        }
    }
    ...
    public void setInMotherTreeZone(boolean inMotherTreeZone, boolean warnPlayer)
    {
        ...
        if (_inMotherTreeZone != inMotherTreeZone)
        {
            ...
            if (warnPlayer)
                ...
                else sendPacket(new SystemMessage(SystemMessage.EXIT_SHADOW_MOTHER_TREE));
        }
    }
    ...
    * Added Config.ADD_MAX_LOAD ~AresXP
                ...
                if (!effectSkill.getWeaponDependancy(this))
                {
                    ...
                    if (Config.DEBUG)
                        _log.info("   | Skill " + effectSkill.getName() + " has been disabled for ("
                            + this.getName() + "); Reason: Incompatible Weapon Type.");
        ...
        if (Config.AUTO_LEARN_SKILLS)
        {
            ...
            while (skills.length > unLearnable)
            {
                for (L2SkillLearn s : skills)
                {
                    ...
                    {
                        if (countUnlearnable) unLearnable++;
                        continue;
                    }
            ...
            this.sendMessage("You have learn new " + skillCounter + " skills.");
        }
    ...
    public Race getRace()
    {
        if (!isSubClassActive()) return getTemplate().race;
    ...
    public L2ItemInstance checkItemManipulation(int objectId, int count, String action)
    {
        ...
        {
            _log.finest(getObjectId() + ": player tried to " + action + " item with invalid count: "
                + count);
            return null;
        }
    ...
    protected void doPickupItem(L2Object object)
    {
        ...
        if (isAlikeDead())
        {
            ...
            _log.warning("trying to pickup when dead.");
            return;
        }
    ...
    */
    ...
    private void onDieDropItem(L2Character killer)
    {
        ...
        if (getKarma() <= 0 && killer instanceof L2PcInstance
            && ((L2PcInstance) killer).getClan() != null && this.getClan() != null
            ...
            while (dropPercent > 0 && Rnd.get(100) < dropPercent && dropCount < dropLimit)
            {
                ...
                for (L2ItemInstance itemDrop : getInventory().getItems())
                {
                    ...
                    if (Rnd.get(100) < itemDropPercent)
                    {
                        ...
                        if (isKarmaDrop) _log.warning(getName() + " has karma and dropped id = "
    ...
    public void deathPenalty(boolean atwar)
    {
        ...
        if (Config.DEBUG) _log.fine(getName() + " died and lost " + lostExp + " experience.");
    ...
    public void onTradeFinish(boolean successfull)
    {
        ...
        if (successfull) sendPacket(new SystemMessage(SystemMessage.TRADE_SUCCESSFUL));
    }
    ...
    {
        if (newSkill.getId() > 369 && newSkill.getId() < 392)
            return;
            ...
            if (this.getClan() != null)
            {
                ...
                if (siege != null)
                {
                    ...
                    if (siege.checkIsDefender(((L2PcInstance) attacker).getClan())
                        && siege.checkIsDefender(this.getClan())) return false;

                    // Check if a siege is in progress and if attacker and the L2PcInstance aren't in the Attacker clan
                    if (siege.checkIsAttacker(((L2PcInstance) attacker).getClan())
                        && siege.checkIsAttacker(this.getClan())) return false;
                }
    ...
    public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
    {
        ...
        */
        ...
        if ((skill.getId() == 13 || skill.getId() == 299)
            && !SiegeManager.getInstance().checkIfOkToSummon(this, false)) return;
        ...
        if (getCurrentSkill() != null && isCastingNow())
        {
            ...
            if (skill.getId() == getCurrentSkill().getSkillId()) return;
        ...
        if (isFishing()
            && (skill.getSkillType() != SkillType.PUMPING && skill.getSkillType() != SkillType.REELING && skill.getSkillType() != SkillType.FISHING))
        {
        ...
        if (skill.isOffensive())
        {
            ...
            if (dontMove)
            {
                ...
                if (skill.getCastRange() > 0
                    && !isInsideRadius(target, skill.getCastRange(), false, false))
                {
                    // Send a System Message to the caster
    ...
    public void rechargeAutoSoulShot(boolean physical, boolean magic, boolean summon)
    {
        ...
        if (_activeSoulShots == null || _activeSoulShots.size() == 0) return;
    ...
    public void savePathNodes(String fileName)
    {
        try
        {
            ...
            sendPacket(SystemMessage.sendString("Path Node table saved to: L2J/pathnode/" + fileName
                + ".bin"));
        }
        catch (Exception e)
        {
    ...
    public void loadPathNodes(String fileName)
    {
        ...
        {
            sendPacket(SystemMessage.sendString("Could not Load Path Node Table from: L2J/pathnode/"
                + fileName + ".bin"));
            e.printStackTrace();
            return;
        }
        ...
        sendPacket(SystemMessage.sendString("Path Node table loaded from: L2J/pathnode/" + fileName
            + ".bin"));
    }
    ...
    public boolean modifySubClass(int classIndex, int newClassId)
    {
        ...
        {
            _log.warning("Could not modify sub class for " + getName() + " to class index " + classIndex
                + ": " + e);
    ...
    public void onPlayerEnter()
    {
        ...
        if (getInvisible() == 1) sendMessage("Entering world in Invisible mode.");
        if (getMessageRefusal()) sendMessage("Entering world in Message Refusal mode.");
    ...
    public void setCurrentSkill(L2Skill currentSkill, boolean ctrlPressed, boolean shiftPressed)
    {
        ...
        {
            if (Config.DEBUG) _log.info("Setting current skill: NULL for " + getName() + ".");
    ...
    public void setQueuedSkill(L2Skill queuedSkill, boolean ctrlPressed, boolean shiftPressed)
    {
        if (queuedSkill == null)
        {
            if (Config.DEBUG) _log.info("Setting queued skill: NULL for " + getName() + ".");
