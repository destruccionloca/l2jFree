   ...
   if npcId == 30568 and st.getInt("cond")==0 and st.getInt("onlyone")==0 : 
      ...
      htmltext = "<html><body>This quest has already been completed.</body></html>" 
          ...
          if st.getInt("id") != 107 : 
            ...
            st.takeItems(LETTER_TO_ELF_ID,1) 
            st.takeItems(HATOSS_ORDER3_ID,1)
            st.giveItems(LESSER_HEALING_ID,int(100)) 
            st.giveItems(BUTCHER_ID,1)
            st.giveItems(CRYSTAL_BATTLE,int(10)) 
            st.giveItems(CRYSTAL_LOVE,int(10)) 
            st.giveItems(CRYSTAL_SOLITUDE,int(10)) 
            st.giveItems(CRYSTAL_FEAST,int(10)) 
            st.giveItems(CRYSTAL_CELEBRATION,int(10))
