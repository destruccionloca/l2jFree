...
class Quest (JQuest) :
 ...
 def onTalk (Self,npc,st):
   ...
   if id == CREATED :
     ...
     st.set("onlyone","0")
     st.set("id","0")
   if npcId == 30118 and int(st.get("cond"))==0 and int(st.get("onlyone"))==0 :
      if st.getPlayer().getClassId().getId() in [ 0x0f,0x2a ] :
         ...
         st.exitQuest(1)
   elif npcId == 30118 and int(st.get("cond"))==0 and int(st.get("onlyone"))==1 :
      htmltext = "<html><head><body>This quest has already been completed.</body></html>"
   elif npcId == 30118 and int(st.get("cond"))==3 and st.getQuestItemsCount(HUGE_NAIL)>=1:
        htmltext = "30118-05.htm"
   elif npcId == 30118 and int(st.get("cond"))>=4 :
        htmltext = "30118-07.htm"
   elif npcId == 30666 and int(st.get("cond"))==4 and st.getQuestItemsCount(LETTER_OF_INTRODUCTION)>0 :
        htmltext = "30666-01.htm"
   elif npcId == 30666 and int(st.get("cond"))==5 and st.getQuestItemsCount(SLAS_LETTER)>0 :
        htmltext = "30666-05.htm"
   elif npcId == 30666 and int(st.get("cond"))==10 :
   ...
   elif npcId == 30668 and (int(st.get("cond"))==5 or int(st.get("cond"))==6) :
   ...
   elif npcId == 30668 and int(st.get("cond"))==8:
        htmltext = "30668-02.htm"
        st.getPcSpawn().addSpawn(27130,-4106,40174,-3660)
   elif npcId == 30668 and int(st.get("cond"))==9 :
        htmltext = "30668-03.htm"
        st.set("cond","10")
        st.giveItems(KATARIS_LETTER,1)
        st.takeItems(LETTER_OF_BETRAYER,1)
   elif npcId == 30732 and int(st.get("cond"))==7 :
        htmltext = "30732-01.htm"
        st.set("cond","8")
        st.giveItems(OLMAHUMS_MONEY,1)
   elif npcId == 30669 and int(st.get("cond"))==11 and st.getQuestItemsCount(GREETINGS)>0 :
        htmltext = "30669-01.htm"
   elif npcId == 30669 and int(st.get("cond"))==12 :
        htmltext = "30669-04.htm"
        st.set("cond","13")
        st.giveItems(KAKANS_LETTER,1)
        st.takeItems(GREETINGS,1)
   elif npcId == 30670 and int(st.get("cond"))==13 and st.getQuestItemsCount(GREETINGS)>0 :
        htmltext = "30670-01.htm"
   elif npcId == 30670 and int(st.get("cond"))==14 and st.getQuestItemsCount(GREETINGS)>0 :
        htmltext = "30670-04.htm"
        st.set("cond","15")
        st.giveItems(NYAKURIS_LETTER,1)
        st.takeItems(GREETINGS,1)
   elif npcId == 30667 and int(st.get("cond"))==15 and st.getQuestItemsCount(GREETINGS)>0 :
   ...
   elif npcId == 30667 and int(st.get("cond"))==16 :
...
CREATED.addTalkId(30118)
STARTING.addTalkId(30118)
COMPLETED.addTalkId(30118)

for npcId in [30118,30666,30667,30669,30670,30732,30668]:
