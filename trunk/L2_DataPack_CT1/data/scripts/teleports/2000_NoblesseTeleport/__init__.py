...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,player):
    ...
    if player.isNoble() == 1 :
      bypass = "<a action=\"bypass -h Quest 2000_NoblesseTeleport noble-nopass.htm"
      if st.getQuestItemsCount(6651):
         bypass = "<a action=\"bypass -h npc_%objectId%_Chat 3"
      htmltext=st.showHtmlFile("noble.htm").replace("%bypass%",str(bypass))
    else :
