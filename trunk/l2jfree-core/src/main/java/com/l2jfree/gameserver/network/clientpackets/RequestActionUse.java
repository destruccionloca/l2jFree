...
public class RequestActionUse extends L2GameClientPacket
{
	...
	private void useSocial(int socialId, L2PcInstance activeChar)
	{
		...
		if (activeChar.getPrivateStoreType() == 0 && activeChar.getActiveRequester() == null && !activeChar.isAlikeDead() && !activeChar.isCastingNow()
				&& !activeChar.isCastingSimultaneouslyNow() && (!activeChar.isAllSkillsDisabled() || activeChar.isInDuel())
				&& activeChar.getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE && FloodProtector.tryPerformAction(activeChar, Protected.SOCIAL))
		{
