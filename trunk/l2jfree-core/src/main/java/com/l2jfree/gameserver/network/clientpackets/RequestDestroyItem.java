...
public class RequestDestroyItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if (itemToRemove.isWear() || ((!itemToRemove.isDestroyable() || CursedWeaponsManager.getInstance().isCursed(itemId)) && !activeChar.isGM()))
		{
			requestFailed(SystemMessageId.CANNOT_DISCARD_THIS_ITEM);
			return;
		}
		...
		{
			sendAF();
			Util.handleIllegalPlayerAction(activeChar, "[RequestDestroyItem] count > 1 but item is not stackable! oid: " + _objectId + " owner: "
