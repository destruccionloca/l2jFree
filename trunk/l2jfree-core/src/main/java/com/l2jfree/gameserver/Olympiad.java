...
public class Olympiad
{
	...
	protected static Map<Integer, FastList<L2PcInstance>>	_classBasedRegisters;
	...
	protected static final int[]	CLASSES		=
			...
			131,
			132,
			//133, // Only one class for soul hound
			134
			//136 // Cannot be chosen as main class
	};
	...
	public void addSpectator(int id, L2PcInstance spectator)
	{
		...
		for (FastList<L2PcInstance> list : _classBasedRegisters.values())
		{
	...
	private class OlympiadManager implements Runnable
	{
					...
					for (int i = 0; i < STADIUMS.length; i++)
					{
						...
						if (STADIUMS[i].isFreeToUse())
						{
							if (existNextOpponents(_nonClassBasedRegisters))
							{
								try
								{
									_olympiadInstances.put(i, new L2OlympiadGame(i, COMP_TYPE.NON_CLASSED, nextOpponents(_nonClassBasedRegisters), STADIUMS[i]
											.getCoordinates()));
							...
							else if (existNextOpponents(getRandomClassList(_classBasedRegisters)))
							{
								try
								{
									_olympiadInstances.put(i, new L2OlympiadGame(i, COMP_TYPE.CLASSED, nextOpponents(getRandomClassList(_classBasedRegisters)),
											STADIUMS[i].getCoordinates()));
									_gamesQueue.put(i, new OlympiadGameTask(_olympiadInstances.get(i)));
									STADIUMS[i].setStadiaBusy();
								}
								catch (Exception ex)
		...
		private FastList<L2PcInstance> getRandomClassList(Map<Integer, FastList<L2PcInstance>> list)
		{
			if (list.size() == 0)
				return null;

			Map<Integer, FastList<L2PcInstance>> tmp = new FastMap<Integer, FastList<L2PcInstance>>();
			int tmpIndex = 0;
		...
		private FastList<L2PcInstance> nextOpponents(FastList<L2PcInstance> list)
		{
		...
		private boolean existNextOpponents(FastList<L2PcInstance> list)
		{
			if (list == null)
				return false;
			if (list.size() == 0)
	...
	private class L2OlympiadGame
	{
		...
		public String					_playerOneIp;
		public String					_playerTwoIp;
		...
		public FastList<L2PcInstance>	_spectators;
		private SystemMessage			_sm;
		private SystemMessage			_sm2;
		...
		protected L2OlympiadGame(int id, COMP_TYPE type, FastList<L2PcInstance> list, int[] stadiumPort)
		{
			...
			if (list != null)
			{
				...
				try
				{
					...
					_playerOne.setOlympiadOpponentId(_playerTwo.getObjectId());
					_playerTwo.setOlympiadOpponentId(_playerOne.getObjectId());
					getPlayersIp();
				}
				catch (Exception e)
				{
		...
		protected void validateWinner()
		{
			...
			case NON_CLASSED:
				...
				_rewardItem = Config.ALT_OLY_NONCLASSED_REWARD_ITEM;
				break;
			default:
				_div = 3;
				_gpreward = Config.ALT_OLY_CLASSED_RITEM_C;
				_rewardItem = Config.ALT_OLY_CLASSED_REWARD_ITEM;
				break;
			}

			if (_playerOneIp.equals(_playerTwoIp) && !Config.ALT_OLY_SAME_IP)
			{
