...
import com.l2jfree.gameserver.model.actor.instance.L2FortMerchantInstance;
...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target == null || target instanceof L2SiegeGuardInstance || target instanceof L2CommanderInstance || target instanceof L2FortMerchantInstance
				|| target instanceof L2FolkInstance || target instanceof L2DoorInstance || target.isAlikeDead())
			return false;
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (npc.getAI() != null) // TODO: possibly check not needed
			{
				...
				//&& _actor.getAttackByList().contains(getAttackTarget())
						...
						&& target.isInsideRadius(npc, 1500, true, false) && GeoClient.getInstance().canSeeTarget(npc, target))
				{
	...
	private void attackPrepare()
	{
		...
		if (!GeoClient.getInstance().canSeeTarget(_actor, _attackTarget))
		{
