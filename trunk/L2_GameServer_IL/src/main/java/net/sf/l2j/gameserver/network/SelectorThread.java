	...
	public void run() {
		try {
			...
			for (;;) {
				...
				{
					sleep(40); // sleep some time to prevent thread from eating all cpu time
	...
	private void readData(SelectionKey sk) {
		...
		catch (IOException e)
		{
			try
			{
                ...
                L2GameClient.saveCharToDisk(con.getClient().getActiveChar());
                if(_log.isDebugEnabled())
		...
		try {
			if (sk.isAcceptable()) {
				...
				if (c != null) {
					...
					L2GameClient client = L2GameClient.create(c.socket());
