	...
	private L2PcInstance getCheckedRandomMember(int ItemId, L2Character target)
	{
		...
		for (L2PcInstance member : getPartyMembers())
		{
			if (member.getInventory().validateCapacityByItemId(ItemId) &&
                    Util.checkIfInRange(Config.PARTY_RANGE, target, member, true)) availableMembers.add(member);
		}
    		...
    		for (L2Character member : rewardedMembers)
    		{
    		    ...
    		    if ((member instanceof L2PlayableInstance && lastAttacker.getKnownList().knowsObject(member)) || lastAttacker.getKnownList().knowsThePlayer((L2PcInstance)member))
    			{
                    ...
                    if (_validMembers.contains(member))
                    {
						...
						member.addExpAndSp(Math.round(member.getStat().calcStat(Stats.EXPSP_RATE, xpReward * preCalculation, null, null)), 
                                           (int)member.getStat().calcStat(Stats.EXPSP_RATE, spReward * preCalculation, null, null));
                    }
    			}
    		}
        }
