    ...
    public void endDuel(int duelId, boolean party, int looser)
    {
        ...
        if (player.getParty()!=null)
        {
            for (L2PcInstance pm : player.getParty().getPartyMembers())
            {
            	...
            	pm.abortCast();
                pm.setDuelling(0);
                pm.getStatus().setCurrentCp(pm.getStat().getMaxCp());
                pm.getStatus().setCurrentHp(pm.getStat().getMaxHp()/2);
                pm.getStatus().setCurrentMp(pm.getStat().getMaxMp()/2);
                pm.setTeam(0);
        ...
        else
        {
            ...
            player.getStatus().setCurrentHp(player.getStat().getMaxHp()/2);
            player.getStatus().setCurrentMp(player.getStat().getMaxMp()/2);
            player.setTeam(0);
