...
public class AdminSortMultisellItems implements IAdminCommandHandler {
	...
	_misc[] = 		{ 	"Scroll" , "Enchant" , "L2Day" , "Potions" , "Hair" , "Pet" , 
						"Quest" , "EchoCrystal" , "Recipe" , "SpellBook" , "Dyes" };
	...
	private class GMObj
	{
		...
		void addItem(int itemId , int price)
		{
            ...
            String addString = (MULTISELL_ID_SORT) ? 
            		(itemId + splitting + itemName + splitting + price):
            		(itemName + splitting + itemId + splitting + price);
		...
		void generateXML()
		{
			...
			for (String string : NameArray)
			{
				...
				try
				{
					itemId = (param.length > 1) ? 
							 Integer.valueOf( (MULTISELL_ID_SORT) ? param[0] : param[1] ).intValue() : 0;					
				}
				catch(Throwable t)
				{
					itemId = 0;
				...
				xml+=(price > 0) ?
	    				"  <!-- ("+itemName+":Id "+itemId+") for ("+price+" adena) --> \n":
	    				"  <!-- ("+itemName+":Id "+itemId+") --> \n";
	...
	private GMObj 			etc , etcQuest , etcEvent , etcSeed , 
							etcHerb , etcMonster , etcMerc , etcFish , 
							spellItems , infinity , unsealed_A_armor,
							unsealed_S_armor, SA_A_weapon, SA_S_weapon ,
							SA_S80_weapon , unsealed_S80_armor;
	...
	private void init()
	{
		for (String s : _weapon)	{	weapon.add(new GMObj(s,"Weapons"));		}
		for (String s : _armor)		{	armor.add(new GMObj(s,"Armors"));		}
		for (String s : _jewelry)	{	jewelry.add(new GMObj(s,"Jewelry"));	}
		for (String s : _misc)		{	misc.add(new GMObj(s,"Miscellaneous"));	}
	}
	...
	private void addMisc(int itemId , int price)
	{
		...
		else if (	item.getItemType() == L2EtcItemType.ARROW ||
					item.getItemType() == L2EtcItemType.BOLT)
		{
			addWeapon(itemId , price);
			return;
	...
	private Vector<L2Item> sortItemsByType(Vector<L2Item> items)
	{
		...
		for (L2WeaponType type : L2WeaponType.values())		
		{
			...
			for (L2Item item : items)			
			{
				...
				L2WeaponType weaponType = ((net.sf.l2j.gameserver.templates.L2Weapon)item).getItemType();
