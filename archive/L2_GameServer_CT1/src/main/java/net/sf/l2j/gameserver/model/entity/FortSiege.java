...
public class FortSiege
{
    ...
    private boolean checkIfCanRegister(L2PcInstance player)
    {
        if (getIsRegistrationOver())
            ...
            || player.getClan().getLevel() < FortSiegeManager.getInstance().getSiegeClanMinLevel())
                player.sendMessage("Only clans with Level " + FortSiegeManager.getInstance().getSiegeClanMinLevel()
                                    + " and higher may register for a fort siege.");
        else if (player.getClan().getHasFort() > 0)
            player.sendMessage("You cannot register because your clan already owns a fort.");
    ...
    private void saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration)
    {
        ...
        {
            _log.warn("Exception: saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration): "
                + e.getMessage());
            e.printStackTrace();
        }
        finally
