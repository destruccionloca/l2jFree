...
import com.l2jfree.gameserver.geodata.loader.GeoFileInfo;
import com.l2jfree.gameserver.geodata.loader.GeoLoader;
import com.l2jfree.gameserver.geodata.loader.GeoLoaderFactory;
...
public class GeoEngine
{
	...
	private static final int			geodataSizeX			= L2World.WORLD_SIZE_X;
	private static final int			geodataSizeY			= L2World.WORLD_SIZE_Y;
	...
	{
		return MoveCheck(x - L2World.MAP_MIN_X >> 4, y - L2World.MAP_MIN_Y >> 4, z, tx - L2World.MAP_MIN_X >> 4, ty - L2World.MAP_MIN_Y >> 4);
	}

	public Location moveCheckForAI(Location loc1, Location loc2)
	{
		return MoveCheckForAI(loc1.getX() - L2World.MAP_MIN_X >> 4, loc1.getY() - L2World.MAP_MIN_Y >> 4, loc1.getZ(), loc2.getY() - L2World.MAP_MIN_X >> 4,
				loc2.getY() - L2World.MAP_MIN_Y >> 4);
	}
	...
	public boolean canSeeTarget(int x, int y, int z, int tx, int ty, int tz)
	{
		...
		return canSee(tx - L2World.MAP_MIN_X >> 4, ty - L2World.MAP_MIN_Y >> 4, tz, x - L2World.MAP_MIN_X >> 4, y - L2World.MAP_MIN_Y >> 4, z);
	}

	public boolean canMoveToTarget(int x, int y, int z, int tx, int ty, int tz)
	{
		return canMove(x - L2World.MAP_MIN_X >> 4, y - L2World.MAP_MIN_Y >> 4, z, tx - L2World.MAP_MIN_X >> 4, ty - L2World.MAP_MIN_Y >> 4, tz);
	}

	public boolean canMoveToTargetWithCollision(int x, int y, int z, int tx, int ty, int tz)
	{
		return canMoveWithCollision(x - L2World.MAP_MIN_X >> 4, y - L2World.MAP_MIN_Y >> 4, z, tx - L2World.MAP_MIN_X >> 4, ty - L2World.MAP_MIN_Y >> 4, tz);
	}
	...
	private Location MoveCheck(int x, int y, int z, int tx, int ty)
	{
		...
		if (dx >= dy) // dy/dx <= 1
		{
			...
			for (int i = 0; i < dx; i++)
			{
				...
				if (d > 0)
				{
					...
					next_z = NcanMoveNext(x, y, z, next_x, next_y);
					if (next_z == 0)
						return new Location((prev_x << 4) + L2World.MAP_MIN_X, (prev_y << 4) + L2World.MAP_MIN_Y, prev_z);
				}
				else
				{
					d += delta_A;
		...
		return new Location((next_x << 4) + L2World.MAP_MIN_X, (next_y << 4) + L2World.MAP_MIN_Y, next_z);
	}
