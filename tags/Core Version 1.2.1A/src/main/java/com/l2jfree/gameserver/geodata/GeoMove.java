...
import com.l2jfree.gameserver.geodata.PathFind.GeoNode;
	...
	public Vector<Location> checkMovement(int x, int y, int z, Location target)
	{
		...
		if (!n.getPath().isEmpty())
		{
			...
			for (GeoNode p : n.getPath())
				targetRecorder.add(new Location((p.location.getX() << 4) + L2World.MAP_MIN_X, (p.location.getY() << 4) + L2World.MAP_MIN_Y, p.location.getZ()));
