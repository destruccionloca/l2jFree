...
public class Siege
{
	...
	// Castle setting
	private FastList<L2ArtefactInstance> _artefacts = new FastList<L2ArtefactInstance>();
	private FastList<L2Spawn> _artefactSpawns = new FastList<L2Spawn>();
	private FastList<L2ControlTowerInstance> _controlTowers = new FastList<L2ControlTowerInstance>();
	private FastList<L2Spawn> _controlTowerSpawns = new FastList<L2Spawn>();
	...
	public void endSiege()
	{
		if(getIsInProgress())
		{
			...
			removeArtefacts(); // Remove artifact from this castle
	...
	{
		_log.error("Siege.announceToPlayer not done!!!");
		/*
		// Get all players
		for(L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			if(!inAreaOnly || (inAreaOnly && getZone().checkIfCharacterInZone(player)))
	...
	public FastList<L2PcInstance> getAttackersInZone()
	{
		...
		for(L2SiegeClan siegeclan : getAttackerClans())
		{
			...
			{
				if(player.getClan() != null && player.getClan().getClanId() == clan.getClanId())
					players.add(player);
			}
		}
		return players;
	...
	public FastList<L2Spawn> getArtefactSpawns()
	{
		if(_artefactSpawns == null)
			_artefactSpawns = new FastList<L2Spawn>();
		return _artefactSpawns;
	}

	/** Get castle artefacts. */
	public FastList<L2ArtefactInstance> getArtefacts()
	{
		if(_artefacts == null)
			_artefacts = new FastList<L2ArtefactInstance>();
		return _artefacts;
	}
	...
	{
		for(L2Spawn spawn : getArtefactSpawns())
		{
			L2ArtefactInstance ar = new L2ArtefactInstance(IdFactory.getInstance().getNextId(), spawn.getTemplate());
			ar.getStatus().setCurrentHpMp(ar.getMaxHp(), ar.getMaxMp());
			ar.setHeading(spawn.getHeading());
			ar.spawnMe(spawn.getLocx(), spawn.getLocy(), spawn.getLocz() + 20);
			getArtefacts().add(ar);
		}
	}
	...
	{
		if(_controlTowerSpawns == null)
			_controlTowerSpawns = new FastList<L2Spawn>();
		return _controlTowerSpawns;
	}

	/** Get castle control towers. */
	private FastList<L2ControlTowerInstance> getControlTowers()
	{
	...
	{
		for(L2Spawn spawn : getControlTowerSpawns())
		{
			L2ControlTowerInstance ct = new L2ControlTowerInstance(IdFactory.getInstance().getNextId(), spawn.getTemplate());
			ct.getStatus().setCurrentHpMp(ct.getMaxHp(), ct.getMaxMp());
			ct.setHeading(spawn.getHeading());
			ct.spawnMe(spawn.getLocx(), spawn.getLocy(), spawn.getLocz() + 20);
			getControlTowers().add(ct);
		}
	}

	/** Remove all control towers spawned. */
	private void removeControlTowers()
	{
		// Remove all instances of control towers for this castle
		for(L2ControlTowerInstance ct : getControlTowers())
		{
			if(ct != null)
				ct.decayMe();
		}

		getControlTowers().clear();
	}
