...
public class PledgeReceiveMemberInfo extends ServerBasePacket
{
    ...
    private L2ClanMember _cm;
    private static Logger _log = Logger.getLogger(ServerBasePacket.class.getName());


	
	public PledgeReceiveMemberInfo(L2ClanMember activeChar)
	{
        _cm = activeChar;
	}	
	...
	final void writeImpl()
	{
		...
		writeS(_cm.getName());
        if (_cm.isOnline())
            writeS(_cm.getPlayerInstance().getTitle());
        else
            writeS(_cm.getTitle());
        writeD(_cm.getRank());//Pledge rank goes here NOT Pledge class
        writeS("");//Relation goes here (don't have a clue what this means though)
        writeS(_cm.getApprentice());//Aprentice's name goes here
	}
