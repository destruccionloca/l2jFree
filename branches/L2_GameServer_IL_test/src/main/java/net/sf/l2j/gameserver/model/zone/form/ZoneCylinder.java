...
{
	private int _x, _y, _rad, _radS;
	private Point3D _min;
	
	public ZoneCylinder(int x, int y, int zMin, int zMax, int rad)
	{
		_x = x;
		_y = y;
		_zMin = zMin;
		...
		_min = new Point3D(x - rad, y - rad, zMin);
	}

	@Override
	public boolean isInsideZone(int x, int y)
	{
		return (Math.pow(_x-x, 2) + Math.pow(_y-y, 2)) < _radS;
	}
	...
	public Location getRandomLocation()
	{
		...
		do
		{
			x = _min.getX() + Rnd.nextInt(_rad * 2);
			y = _min.getY() + Rnd.nextInt(_rad * 2);
		}
