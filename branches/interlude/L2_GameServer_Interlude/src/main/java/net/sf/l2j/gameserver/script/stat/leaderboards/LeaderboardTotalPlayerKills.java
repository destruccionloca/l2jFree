 ...
 * @author  Imper1um
 */
public class LeaderboardTotalPlayerKills implements Leaderboard {
   private static String leaderboardDescription = "Total Player Kills";
   private int id = -1;
   private FastList<LeaderboardEntry> _leaderboard;
   
   public LeaderboardTotalPlayerKills() {
   ...
   public void update(Connection c, ResultSet rs) throws SQLException {
       ...
       while (rs.next()) {
           if (_leaderboard.getLast() == null) { //No Entries, yet.
               ...
               _leaderboard.add(new LeaderboardEntry(s, rs.getInt("totalPlayerKills")));
           } else {
               int i = -1;
               int sc = rs.getInt("totalPlayerKills");
               boolean pass = true;
