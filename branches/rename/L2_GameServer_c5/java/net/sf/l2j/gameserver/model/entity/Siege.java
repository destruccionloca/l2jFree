...
public class Siege
{
	...
	public class ScheduleEndSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_Castle), timeRemaining - 3600000); // Prepare task for 1 hr left.
				}
				else if ((timeRemaining <= 3600000) && (timeRemaining > 600000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_Castle), timeRemaining - 600000); // Prepare task for 10 minute left.
				}
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_Castle), timeRemaining - 300000); // Prepare task for 5 minute left.
				}
				else if ((timeRemaining <= 300000) && (timeRemaining > 10000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_Castle), timeRemaining - 10000); // Prepare task for 10 seconds count down
				}
				else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					announceToPlayer(getCastle().getName() + " siege " + Math.round(timeRemaining / 1000) + " second(s) left!", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_Castle), timeRemaining); // Prepare task for second count down
				}
				else
				{
	...
	public class ScheduleStartSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining - 86400000); // Prepare task for 24 before siege start to end registration
				}
				else if ((timeRemaining <= 86400000) && (timeRemaining > 13600000))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining - 13600000); // Prepare task for 1 hr left before siege start.
				}
				else if ((timeRemaining <= 13600000) && (timeRemaining > 600000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege begin.", false);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining - 600000); // Prepare task for 10 minute left.
				}
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege begin.", false);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining - 300000); // Prepare task for 5 minute left.
				}
				else if ((timeRemaining <= 300000) && (timeRemaining > 10000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000) + " minute(s) until " + getCastle().getName() + " siege begin.", false);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining - 10000); // Prepare task for 10 seconds count down
				}
				else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					announceToPlayer(getCastle().getName() + " siege " + Math.round(timeRemaining / 1000) + " second(s) to start!", false);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_Castle), timeRemaining); // Prepare task for second count down
				}
				else
				{
	...
	public void midVictory()
	{
		if (getIsInProgress())                          // Siege still in progress
		{
			...
			if      (
					getDefenderClans().size() == 0 &&   // If defender doesn't exist (Pc vs Npc)
			...
			{
				if (_IsNormalSide)
					_IsNormalSide = false;              // Atk is now def
				else
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			...
			{
				if (getCastle().getOwnerId() <= 0)
					announceToPlayer("The siege of " + getCastle().getName() + " has been canceled due to lack of interest.", false);
				else
					announceToPlayer(getCastle().getName() + "'s siege was canceled because there were no clans that participated.", false);
				return;
			}
	...
	public void announceToPlayer(String message, boolean inAreaOnly)
	{
		...
		{
			if (!inAreaOnly || (inAreaOnly && checkIfInZone(player.getX(), player.getY()))) player.sendMessage(message);
		}
	}
	...
	public boolean checkIfInZone(int x, int y)
	{
		return (getIsInProgress() &&
				(
						getCastle().checkIfInZone(x, y) ||                              // Castle Zone
	 ...
	 */
	public boolean checkIsAttacker(L2Clan clan) { return (getAttackerClan(clan) != null); }

	/**
	 * Return true if clan is defender<BR><BR>
	 * @param clan The L2Clan of the player
	 */
	public boolean checkIsDefender(L2Clan clan) { return (getDefenderClan(clan) != null); }

	/**
	 * Return true if clan is defender waiting approval<BR><BR>
	 * @param clan The L2Clan of the player
	 */
	public boolean checkIsDefenderWaiting(L2Clan clan) { return (getDefenderWaitingClan(clan) != null); }
	...
	public List<L2PcInstance> getAttackersInZone()
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			if (player.getClan() != null && getAttackerClan(player.getClan()) != null && checkIfInZone(player.getX(), player.getY()))
				players.add(player);
		}
	...
	public List<L2PcInstance> getDefendersInZone()
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			...
			if (player.getClan() != null && getDefenderClan(player.getClan()) != null && checkIfInZone(player.getX(), player.getY()))
				players.add(player);
		}
	...
	public List<L2PcInstance> getOwnersInZone()
	{
		...
		if (ownerId > 0)
		{
			for (L2PcInstance player : L2World.getInstance().getAllPlayers())
			{
				if (player.getClan() != null && player.getClan().getClanId() == ownerId && checkIfInZone(player.getX(), player.getY()))
					players.add(player);
			}
		}
	...
	public List<L2PcInstance> getSpectatorsInZone()
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			if (player.getClan() == null || (getAttackerClan(player.getClan()) == null && getDefenderClan(player.getClan()) == null && checkIfInZone(player.getX(), player.getY())))
				players.add(player);
		}
	...
	public void listRegisterClan(L2PcInstance player) { player.sendPacket(new SiegeInfo(getCastle())); }

	/**
	 * Register clan as attacker<BR><BR>
	 * @param player The L2PcInstance of the player trying to register
	 */
	public void registerAttacker(L2PcInstance player) { registerAttacker(player, false); }
	 ...
	 * @param player The L2PcInstance of the player trying to register
	 */
	public void registerDefender(L2PcInstance player) { registerDefender(player, false); }
	...
	{
		if (clan == null || clan.getHasCastle() == getCastle().getCastleId() || !SiegeManager.getInstance().checkIsRegistered(clan)) return;
	 ...
	 * @param player The L2PcInstance of player/clan being removed 
	 */
	public void removeSiegeClan(L2PcInstance player) { removeSiegeClan(player.getClan()); }
	...
	private void loadCastleSiege()
	{
		try
		{
			...
			_ControlTowerSpawnList[0]	= parseCoords(siegeSettings.getProperty(getCastle().getName() + "ControlTower1","0,0,0"));
			_ControlTowerSpawnList[1]	= parseCoords(siegeSettings.getProperty(getCastle().getName() + "ControlTower2","0,0,0"));
			_ControlTowerSpawnList[2]	= parseCoords(siegeSettings.getProperty(getCastle().getName() + "ControlTower3","0,0,0"));
			_ControlTowerSpawnList[3]	= parseCoords(siegeSettings.getProperty(getCastle().getName() + "ControlTower4","0,0,0"));
		}
		catch (Exception e)
		{
	...
	private void saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration)
	{
		...
		try
		{
			...
			{
				if (getDefenderClans().size() + getDefenderWaitingClans().size() >= SiegeManager.getInstance().getDefenderMaxClans()) return;
			}
			else
			{
				if (getAttackerClans().size() >= SiegeManager.getInstance().getAttackerMaxClans()) return;
			}
	...
	{
		_Artifact = new L2ArtefactInstance(IdFactory.getInstance().getNextId(), NpcTable.getInstance().getTemplate(SiegeManager.getInstance().getArtifactSpawnList()[getCastle().getCastleId()][4]));
		...
		_Artifact.spawnMe(SiegeManager.getInstance().getArtifactSpawnList()[getCastle().getCastleId()][0], SiegeManager.getInstance().getArtifactSpawnList()[getCastle().getCastleId()][1], SiegeManager.getInstance().getArtifactSpawnList()[getCastle().getCastleId()][2] + 50);
	}
	...
	private void spawnControlTower()
	{
		...
		for (int i = 0; i < _ControlTowerSpawnList.length; i++)
		{
			...
			ct = new L2ControlTowerInstance(IdFactory.getInstance().getNextId(), new L2NpcTemplate(newStatSet));
			ct.setCurrentHpMp(ct.getMaxHp(), ct.getMaxMp());
			ct.spawnMe(_ControlTowerSpawnList[i][0], _ControlTowerSpawnList[i][1], _ControlTowerSpawnList[i][2] + 20);
	...
	{
		if (_Artifact == null) {loadCastleSiege();}
		return _Artifact;
	}
	...
	public final int getAttackerRespawnDelay() { return (SiegeManager.getInstance().getAttackerRespawnDelay()); }
	...
	public final List<L2SiegeClan> getDefenderWaitingClans() { return _Defender_Waiting_Clans; }

	public final int getDefenderRespawnDelay() { return (SiegeManager.getInstance().getDefenderRespawnDelay()); }

	public final boolean getIsInProgress() { return _IsInProgress; }

	public final boolean getIsRegistrationOver() { return _IsRegistrationOver; }

	public final Calendar getSiegeDate() { return getCastle().getSiegeDate(); }
	...
	{
		if (_SiegeGuardManager == null) {_SiegeGuardManager = new SiegeGuardManager(getCastle());}
		return _SiegeGuardManager;
	}

	public final Zone getZone() { return ZoneManager.getInstance().getZone(ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.SiegeBattleField), getCastle().getName()); }
